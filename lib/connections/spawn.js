...
const spawnConnection = (connectionObject, fn, cb__spawnConnection) => {
  ...
  if (connectionObject.connection.pool) {
    return connectionObject.connection.pool.getConnection()
    .then((conn) => {
      afterwards(null, conn);
    })
    .catch((err) => {
      afterwards(err, null);
    });
  ...
  function afterwards(err, liveConnection) {
    ...
    if (err) {
      ...
      // ("could not connect")
      //
      err = new Error( 'Could not connect to MariaDB:\n' + err.toString());
    ...
    // with the live MariaDB connection (probably send a query).
    if(connectionObject.config.timezone){
      liveConnection.query("SET time_zone = '"+connectionObject.config.timezone+"'");
    }
